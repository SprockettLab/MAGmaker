...
output/
├── qc/
│   ├── fastqc_pre_trim/ fastqc_post_trim/ fastqc_post_host/   per-(sample,run,read) FastQC reports
│   ├── fastp/                            fastp JSON + HTML reports (single-end reports under fastp/se/)
│   ├── host_filter/nonhost/              host-filtered FASTQ files (input to everything downstream)
│   └── multiqc/multiqc.html             combined QC report
│
├── assemble/
│   ├── megahit/{sample}.contigs.fasta      MEGAHIT assemblies
│   ├── metaspades/{sample}.contigs.fasta   metaSPAdes assemblies
...
│
├── profile/                            (read-level profiling, optional)
│   ├── metaphlan/
│   │   ├── profiles/{sample}.txt        per-sample MetaPhlAn 4 profiles
│   │   └── merged_abundance_table.txt   merged taxonomy table (all samples)
│   ├── kraken2/{sample}.report.txt      Kraken2 reports (+ Bracken redistributions)
│   └── krona/{sample}.report.html       Krona interactive plots
│
├── mapping/{mapper}/sorted_bams/        (binning pipeline) sorted, indexed BAMs — differential coverage
│
├── binning/{binner}/{mapper}/{sample}/  (binning pipeline) per-binner draft bins
│
├── selected_bins/{mapper}/             (binning pipeline)
│   ├── DAS_Tool_Fastas/{sample}/   or   Binette_Fastas/{sample}/   consolidated MAGs per sample
│   └── {binner}/scaffolds2bin/{sample}_scaffolds2bin.tsv           per-binner contig→bin tables
│
├── virus/                              (binning pipeline, virus_all target)
│   ├── genomad/{assembler}/{sample}/   viral + plasmid contig classification
│   ├── checkv/{assembler}/{sample}/    CheckV quality for the viral contigs
│   └── virus_summary.tsv               merged per-sample viral + plasmid table
│
└── mag_qc/                             (binning pipeline)
    ├── checkm2/{mapper}/{sample}/      CheckM2 quality reports
    ├── gunc/{mapper}/{sample}/         GUNC chimera detection results
    ├── gtdbtk/{mapper}/{sample}/       GTDB-Tk output (.identify.done / .align.done / .done sentinels)
    ├── cmseq/{mapper}/{sample}/        CMSeq strain-heterogeneity per MAG
...
| `Original_Name` | Bin name from the consolidation tool |
...
| `Winning_Binner` | Which binner produced the selected bin (metabat2 / maxbin2 / concoct / semibin2); for Binette this can be a combination like `metabat2+concoct`, or `binette` for a bin it constructed |
...
`output/virus/virus_summary.tsv` (from the `virus_all` target) has one row per
viral or plasmid contig geNomad classified, with CheckV quality joined on for
the viral rows.

| Column | Description |
|---|---|
| `Sample_ID`, `Assembler` | which assembly the contig came from |
| `Element_Type` | `virus` or `plasmid` |
| `Seq_Name` | contig id |
| `Length_BP`, `Topology`, `N_Genes`, `N_Hallmarks` | geNomad contig stats |
| `Score`, `FDR` | geNomad virus/plasmid score and false-discovery rate |
| `Taxonomy` | geNomad viral taxonomy (virus rows) |
| `Conjugation_Genes`, `AMR_Genes` | geNomad plasmid annotations (plasmid rows) |
| `checkv_quality`, `miuvig_quality` | CheckV quality tier / MIUViG tier (virus rows) |
| `checkv_completeness`, `checkv_contamination` | CheckV estimates (virus rows) |
| `provirus`, `warnings` | CheckV flags (virus rows) |

CheckV does not score plasmids, so those columns are blank on plasmid rows.
See [Viral / plasmid track](viral.md).
