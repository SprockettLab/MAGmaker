...
dir.create("MAGmaker/raw_data/")
scp_list <- c(rbind(unit_df$R1, unit_df$R2))
message("Transferring ", length(scp_list), " fastq files to MAGmaker/raw_data/ using scp.")

for (i in seq_along(scp_list)) {
  i_file <- scp_list[i]
  i_cmd <- paste0("scp ", username, "@", server, ":", i_file, " MAGmaker/raw_data/")
  system(i_cmd)
}
file_list <- list.files("MAGmaker/raw_data/")
if (length(scp_list) == length(file_list)) {
  message("All ", length(scp_list), " files downloaded correctly.")
}

message("Moving samples.txt and updated units.txt to MAGmaker/resources/config/")
sample_cp_cmd <- paste0("cp ", sample_file, " MAGmaker/resources/config/samples.txt")
...
write.table(updated_units_df, file = "MAGmaker/resources/config/units.txt", quote = FALSE, sep = '\t', row.names = FALSE)

setwd("MAGmaker")

message("Begin running MAGmaker.")
